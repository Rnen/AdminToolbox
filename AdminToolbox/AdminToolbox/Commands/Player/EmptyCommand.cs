		...
		public string[] OnCall(ICommandSender sender, string[] args)
		{
			if(sender.IsPermitted(CommandAliases, out string[] deniedReply))
			{
				...
				if (args.Length > 0)
				{
					...
					{
						foreach (SMItem item in myPlayer.GetInventory().Where(i => i.ItemType != ItemType.NULL))
						...
						{
							foreach (SMItem item in myPlayer.GetInventory())
				...
				else if (sender is Player p && p != null)
				{
					...
					foreach (SMItem item in p.GetInventory().Where(i => i.ItemType != ItemType.NULL))
					{ item.Drop(); itemNumber++; }
					Map.SpawnItem(ItemType.DROPPED_5, pos, rot);
					Map.SpawnItem(ItemType.DROPPED_7, pos, rot);
					Map.SpawnItem(ItemType.DROPPED_9, pos, rot);
