		...
		public string[] OnCall(ICommandSender sender, string[] args)
		{
			if (sender.IsPermitted(CommandAliases, out string[] deniedReply))
			{
				if (Server.GetPlayers().Count > 0)
				{
					...
					int remainingJailTime = ((int)playerDict.JailReleaseTime.Subtract(DateTime.UtcNow).TotalSeconds >= 0) ? (int)playerDict.JailReleaseTime.Subtract(DateTime.UtcNow).TotalSeconds : 0;
